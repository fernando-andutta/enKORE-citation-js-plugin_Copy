...
const { plugins } = require("../../../test/api.js")(
				...
				describe(name, function () {
					...
					it("parses data", async function () {
						...
						assert.deepStrictEqual(
							method(
								input,
								Object.assign({ generateGraph: false }, opts || {}),
							),
							expected,
						);
					});
...
describe("configuration", function () {
	describe("langs", function () {
		...
		it("changes default", function () {
			...
			assert.strictEqual(plugins.input.chain(qid)[0].title, "宇宙");
			config.langs.shift();
		});
		it("falls back", function () {
			config.langs.unshift("foo");
			assert.strictEqual(plugins.input.chain(qid)[0].title, "universe");
			config.langs.shift();
		});
